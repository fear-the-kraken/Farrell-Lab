...
class CSD(object):
    ...
    def filter_csd(self, csd, filterfunction='convolve'):
        ...
        if self.f_type == 'boxcar':
            num = scipy.signal.boxcar(self.f_order)
            denom = np.array([num.sum()])
        elif self.f_type == 'hamming':
            num = scipy.signal.hamming(self.f_order)
            denom = np.array([num.sum()])
        elif self.f_type == 'triangular':
            num = scipy.signal.triang(self.f_order)
            denom = np.array([num.sum()])
        elif self.f_type == 'gaussian':
            num = scipy.signal.windows.gaussian(self.f_order[0], self.f_order[1])
            denom = np.array([num.sum()])
        ...
        if filterfunction == 'filtfilt':
            ...
            for i in range(csdf.shape[1]):
                csdf[:, i] = scipy.signal.convolve(csdf[:, i], num/denom.sum(), 'same')
